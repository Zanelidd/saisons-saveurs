import type { SpoonacularRecipe, SpoonacularNutrient, DayMenu, MealEntry, RecipeDetail, Macros } from "@/types";
...
export function buildMenuFromRecipes(
    ...
    return Array.from({ length: days }, (_, i) => ({
        jour: DAY_NAMES[i],
        repas: {
            déjeuner: toMealEntry(shuffled[i * 2]       ?? shuffled[i % shuffled.length]),
            dîner:    toMealEntry(shuffled[i * 2 + 1]   ?? shuffled[(i + 1) % shuffled.length]),
        },
    }));
}
